...
#define radioType 1//IF SET 1 VHF IF SET 0 UHF
...
boolean Calculate_Frequency (char mFRQ[9]) {
  ...
  else if (radioType == 0)
  {
    //UHF
    calc_frequency = ((mFRQ[0] - 48) * 100000L) + ((mFRQ[1] - 48) * 10000L)  + ((mFRQ[2] - 48) * 1000) + ((mFRQ[4] - 48) * 100) + ((mFRQ[5] - 48) * 10) + (mFRQ[6] - 48);
    if ((calc_frequency >= 406000L) & (calc_frequency <= 470000L)) return true; //valid frequency
    else
    {
      Alert_Tone(ERR_tone);
      return false; //invalid frequency
...
//uhf band select from - aselsan manual
//BS1 BS0
// 1 1    406   418
// 1 0    418   430
// 1 1    440   455
// 1 0    455   470
void SelectBandForVHF(unsigned long Frequency)
{
  if ((Frequency < 174000L) & Frequency >= (164000L))  {
    digitalWrite(BAND_SELECT_1, LOW);  digitalWrite(BAND_SELECT_0, LOW);//BAND 4
  }
  if ((Frequency < 164000L) & Frequency >= (154000L))  {
    digitalWrite(BAND_SELECT_1, HIGH);  digitalWrite(BAND_SELECT_0, LOW);//BAND 3
  }
  if ((Frequency < 154000L) & Frequency >= (144000L))  {
    digitalWrite(BAND_SELECT_1, LOW); digitalWrite(BAND_SELECT_0, HIGH); //BAND 2
  }
  if ((Frequency < 146000L) & Frequency >= (134000L))  {
    digitalWrite(BAND_SELECT_1, HIGH); digitalWrite(BAND_SELECT_0, HIGH); //BAND 1
  }
}

void SelectBandForUHF(unsigned long Frequency)
{
  if ((Frequency < 470000L) & Frequency >= (455000L))  {
    digitalWrite(BAND_SELECT_1, HIGH); digitalWrite(BAND_SELECT_0, LOW); //ÜSTBAND 2
  }
  if ((Frequency < 455000L) & Frequency >= (440000L))  {
    digitalWrite(BAND_SELECT_1, HIGH); digitalWrite(BAND_SELECT_0, HIGH); //ÜSTBAND 1
  }
  if ((Frequency < 440000L) & Frequency >= (418000L))  {
    digitalWrite(BAND_SELECT_1, HIGH); digitalWrite(BAND_SELECT_0, LOW);  //ALTBAND 2
  }
  if ((Frequency < 418000L) & Frequency >= (406000L))  {
    digitalWrite(BAND_SELECT_1, HIGH); digitalWrite(BAND_SELECT_0, HIGH);//ALTBAND 1
  }
}

void write_FRQ(unsigned long Frequency) {
  if (validFRQ) {
    if (radioType == 1){ SelectBandForVHF(Frequency); }
    else if (radioType == 0){ SelectBandForUHF(Frequency);}
